...
`isMockEnabled()` returns `true` automatically when `NODE_ENV === 'test'`. No explicit call to `enableMock()` is needed — any provider with `.andMock()` will use the mock.

To test different behaviors within a suite, swap the mock per test with `.andMock()`:
...
describe('getUser', () => {
  beforeAll(() => getUser.andMock(mock.getUser));
...
```ts
import { enableMock, disableMock } from '@rstackio/services/mock';
...
```ts
import { enableMock, isMockEnabled } from '@rstackio/services/mock';
